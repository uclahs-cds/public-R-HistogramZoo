...
### The Union Method
The most commonly used method is taking the union of the regions identified across multiple samples.

### Corces et al., 2018
In [Corces et al., 2018](https://science.sciencemag.org/content/362/6413/eaav1898.abstract), a 'daisy chain' approach was implemented to merge peaks called on ATAC-seq data. In a set of overlapping peaks, the peak with the lowest p-value is used as the representative peak.

### Dirichlet Process Gaussian Mixture Models
In [unpublished work], a Gaussian mixture model (GMM) with a Dirichlet Process prior is fit on the pile-up of bed files. We are using the implementation of the GMM from the R package [DirichletProcess](https://cran.r-project.org/web/packages/dirichletprocess/index.html). 

```{r pipeline, echo=FALSE, fig.cap="Dirichlet Process Gaussian Mixture Models", out.width = '60%'}
...
6) `DP.RESOLUTION` If the METHOD is 'dpc', specify the resolution of the provided PEAKS data.frame. This usually corresponds to the bin width chosen when the peaks are called, a positive integer.

7) `DP.ITERATIONS` If the METHOD is 'dpc', specify the number of iterations used to fit the GMM, a positive integer.

8) `DP.WEIGHT.THRESHOLD` If the METHOD is 'dpc', specify the minimum weight used to threshold the Gaussians. This value is required to be numeric and between 0 and 1.

9) `DP.N.SD` If the METHOD is 'dpc', specify the standard deviation of the Gaussians required to identify peak boundaries, numeric value, above 0.

10) `DP.ALPHA.PRIORS` If the METHOD is 'dpc', specify the priors (alpha, beta) for the Gamma distribution from which the weight concentration prior is drawn, numeric vector of length 2

11) `DP.SEED` A seed for reproducibility, integer above 0.

12) `PLOT.MERGED.PEAKS` Either a logical value (TRUE or FALSE) indicating all or none of the merged peaks should be plotted. Otherwise, a character vector of genes whose merged peaks should be plotted.

13) `OUTPUT.TAG` A character string added to the names of any output files.

14) `OUTPUTDIR` Output directory. If the directory does not exist, ConsensusPeaks will attempt to create the directory.

15) `WRITE.OUTPUT` A logical value indicating whether the output table should be written. Otherwise, it is returned.
...
## Parameter Tuning for the GMM Model

### Alpha Priors

The alpha parameter, or weight concentration parameter, is sampled from a Gamma prior with parameters alpha and beta rather than specified by the user in the DirichletProcess R package. We recommend tuning these parameters to fit the properties of your data. The default parameter settings are alpha = 5 and beta = 0.5 fits a sparse model. The three diagrams below use alpha and beta parameters of ($\alpha = 1$, $\beta = 1$), ($\alpha = 1$, $\beta = 2$) and ($\alpha = 0.5$, $\beta = 5$) respectively. As one can observe, there are significantly more Gaussians fitted to a lower weight concentration parameter. 

```{r alpha prior tuning peaks, echo=FALSE, out.width = '30%'}
knitr::include_graphics("ENSGXX.A1B1.MergedPeaks.png")
knitr::include_graphics("ENSGXX.A1B2.MergedPeaks.png")
knitr::include_graphics("ENSGXX.MergedPeaks.png")
```
