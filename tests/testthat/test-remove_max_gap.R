...
test_that("remove_max_gaps works a single segment", {
  start.end.points = index_to_start_end( c(3, 8) )
  mgaps = data.frame("start" = c(3,7), "end" = c(4, 8))
  p.pairs = remove_max_gaps(start.end.points = start.end.points, max.gaps = mgaps, remove.short.segment = 1)

  expect_equal(p.pairs$start, 5)
  expect_equal(p.pairs$end, 6)

  # Should also work with an empty data frame
  p.pairs.empty = remove_max_gaps(start.end.points = start.end.points, max.gaps = data.frame(), remove.short.segment = 1)
...
test_that("remove_max_gaps works on multiple points", {
  start.end.points = index_to_start_end( c(1, 5, 10) )
  ...
  p.pairs.no.remove = remove_max_gaps(start.end.points = start.end.points, max.gaps = mgaps.short, remove.short.segment = 0)
  p.pairs.remove = remove_max_gaps(start.end.points = start.end.points, max.gaps = mgaps.short, remove.short.segment = 1)
