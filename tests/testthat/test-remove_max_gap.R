test_that("remove_max_gaps works a single segment", {
  ...
  p.pairs = remove_max_gaps(p = p, max.gaps = mgaps, remove.short.segment = 1)

  expect_equal(p.pairs$start, 5)
  expect_equal(p.pairs$end, 6)

  # Should also work with an empty data frame
  p.pairs.empty = remove_max_gaps(p = p, max.gaps = data.frame(), remove.short.segment = 1)
...
test_that("remove_max_gaps works on multiple points", {
  ...
  p.pairs.no.remove = remove_max_gaps(p = p, max.gaps = mgaps.short, remove.short.segment = 0)
  p.pairs.remove = remove_max_gaps(p = p, max.gaps = mgaps.short, remove.short.segment = 1)
